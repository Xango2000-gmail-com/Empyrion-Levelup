...
namespace LevelUp
{
    ...
    {
        public static string ModVersion = "LevelUp v1.0.0";
        public static string ModPath = "..\\Content\\Mods\\LevelUp\\";
            ...
            try
            {
                switch (cmdId)
                {
                    case CmdId.Event_ChatMessage:
                        ...
                        else if (msg == SetupYamlData.LevelUpCommand.ToLower() + "?")
                        {
                            API.Chat("Player", Received_ChatInfo.playerId, "you instantly get bumped to level 25 for a cost of " + SetupYamlData.Cost);
                        }
                        else if (msg == SetupYamlData.LevelUpCommand.ToLower())
                        {
                            try
                    ...
                    case CmdId.Event_DialogButtonIndex:
                        ...
                        if (SeqNrStorage.Keys.Contains(seqNr))
                        {
                            ...
                            if (RetrievedData.Requested == "DialogBox" && RetrievedData.function == "MaxLevel" && Convert.ToString(Received_DialogButtonIndex.Id) == RetrievedData.Match)
                            {
                                ...
                                string EntityID = EntityIDArray[1];
                                if (Received_DialogButtonIndex.Value == 0)
                                {
                                    API.SetLevel(RetrievedData.TriggerPlayer.clientId, 25);
                                    API.Credits(RetrievedData.TriggerPlayer.entityId, -SetupYamlData.Cost);
                                }
                                else
                                {
